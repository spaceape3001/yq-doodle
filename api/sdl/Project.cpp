...
#include <sdl/strings.hpp>
#include <sdl/logging.hpp>
...
namespace yq::sdl {
    ...
    bool     Project::census(Census&, const std::filesystem::path& fp)
    {
        ...
        sdlError << "xml reading not yet implemented";
        return false;
    }
    ...
    bool         Project::read(const XmlDocument&prj)
    {
        const XmlNode* root = prj.first_node(szD3X);
        if(!root){
            sdlError << szExtXML << " does not contain the root element";
            return false;
        }
        ...
        for(const XmlNode* x = root->first_node(szAttribute); x; x = x->next_sibling(szAttribute)){
            ...
            if(!ti){
                sdlWarning << "unable to resolve attribute type " << t;
                continue;
            }
        ...
        for(const XmlNode* x = root->first_node(szObject); x; x = x->next_sibling(szObject)){
            ...
            if(cls.empty()){
                sdlWarning << "unspecified class for object";
                continue;
            }
            auto id = read_attribute(*x, szID, x_uint32);
            if(!id){
                sdlWarning << "unable to read the ID for object (" << cls << ")";
                continue;
            }
            
            const DObjectMeta*   dinfo  = DObjectMeta::lookup(cls);
            if(!dinfo){
                sdlWarning << "unable to find an object type " << cls;
                continue;
            }
            ...
            if(!obj){
                sdlWarning << "unable to create an object type " << cls;
                continue;
            }
            ...
            for(const XmlNode* v = x->first_node(szValue); v; v = v->next_sibling(szValue)){
                ...
                const TypeMeta* ti  = t.empty() ? &meta<std::string>() : TypeMeta::find(t);
                if(!ti){
                    sdlWarning << "unable to resolve value type " << t;
                    continue;
                }
            ...
            for(const XmlNode* p = x->first_node(szProperty); p; p = p->next_sibling(szProperty)){
                ...
                if(!pi){
                    sdlWarning << "unable to find property (" << k << ") on type " << cls;
                    continue;
                }
                ...
                if(!t.empty()){
                    ti  = TypeMeta::find(t);
                    if(!ti){
                        sdlWarning << "Unable to find type info for " << t;
                        continue;
                    }
                }
    ...
    bool         Project::load_xml(ByteArray& bytes, generator_fn&& fn)
    {
        ...
        try {
            ...
            sdlError << "Xml parse error: " << pe.what() << " (at byte " << pt << ")";
            return false;
        }
    ...
    {
        if(!file_exists(fp)){
            sdlError << "File does not exist.  " << fp.string();
            return false;
        }
        
        if(!file_readable(fp)){
            sdlError << "File is not readable.  " << fp.string();
            return false;
        }
        
        ByteArray   bytes   = file_bytes(fp);
        if(bytes.empty()){
            sdlError << "File is empty.  " << fp.string();
            return false;
        }
