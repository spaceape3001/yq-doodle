...
#include <diagram/strings.hpp>
#include <diagram/logging.hpp>
...
namespace yq::diagram {
    ...
    Any        Project::attribute(const std::string&k) const
    {
    ...
    bool     Project::census_xml(Census&, const std::filesystem::path& fp)
    {
        ...
        if(ec != std::error_code())
            return false;
        const XmlNode* root = prj.first_node(szSDX);
        if(!root)
            return false;
        
        diagramError << "xml census reading not yet implemented";
        return false;
    }
    ...
    bool         Project::read(const XmlDocument&prj)
    {
        const XmlNode* root = prj.first_node(szSDX);
        if(!root){
            diagramError << szExtXML << " does not contain the root element";
            return false;
        }
        ...
        for(const XmlNode* x = root->first_node(szAttribute); x; x = x->next_sibling(szAttribute)){
            ...
            if(!ti){
                diagramWarning << "unable to resolve attribute type " << t;
                continue;
            }
        ...
        for(const XmlNode* x = root->first_node(szObject); x; x = x->next_sibling(szObject)){
            ...
            if(cls.empty()){
                diagramWarning << "unspecified class for object";
                continue;
            }
            auto id = read_attribute(*x, szID, x_uint32);
            if(!id){
                diagramWarning << "unable to read the ID for object (" << cls << ")";
                continue;
            }
            
            const DObjectMeta*   dinfo  = DObjectMeta::lookup(cls);
            if(!dinfo){
                diagramWarning << "unable to find an object type " << cls;
                continue;
            }
            ...
            if(!obj){
                diagramWarning << "unable to create an object type " << cls;
                continue;
            }
            ...
            for(const XmlNode* v = x->first_node(szValue); v; v = v->next_sibling(szValue)){
                ...
                const TypeMeta* ti  = t.empty() ? &meta<std::string>() : TypeMeta::find(t);
                if(!ti){
                    diagramWarning << "unable to resolve value type " << t;
                    continue;
                }
            ...
            for(const XmlNode* p = x->first_node(szProperty); p; p = p->next_sibling(szProperty)){
                ...
                if(!pi){
                    diagramWarning << "unable to find property (" << k << ") on type " << cls;
                    continue;
                }
                ...
                if(!t.empty()){
                    ti  = TypeMeta::find(t);
                    if(!ti){
                        diagramWarning << "Unable to find type info for " << t;
                        continue;
                    }
                }
    ...
    bool         Project::load_xml(ByteArray& bytes, generator_fn&& fn)
    {
        ...
        try {
            ...
            diagramError << "Xml parse error: " << pe.what() << " (at byte " << pt << ")";
            return false;
        }
    ...
    {
        XmlNode& root    = *doc.create_element(szSDX);
    ...
    ////////////////////////////////////////////////////////////////////////////
    //  IO (High Level)
    bool   Project::census(Census&ret, const std::filesystem::path&fp)
    {
    ...
    {
        if(!file_exists(fp)){
            diagramError << "File does not exist.  " << fp.string();
            return false;
        }
        
        if(!file_readable(fp)){
            diagramError << "File is not readable.  " << fp.string();
            return false;
        }

        ByteArray   bytes   = file_bytes(fp);
        if(bytes.empty()){
            diagramError << "File is empty.  " << fp.string();
            return false;
        }
